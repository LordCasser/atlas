# mcp-action-aware-tool-schemas Specification

## Purpose
定义 `domain_rules` 与 `fp_dispatches` 的动作级机器可读输入契约，使 JSON Schema 2020-12 客户端能提前识别必填参数，同时保持默认读操作、服务端运行时校验和 MCP 工具目录兼容。
