...
迁移到无损 schema 边界后，Atlas MUST 保持当前 15 项工具的名称、顺序、描述和现有输入 schema 内容不变，并 MUST 保持 `tools/call` 行为不变。
...
- **WHEN** 测试或下游代码通过完整 schema object 读取 `properties` 与 `required`
- **THEN** 获得的参数集合和必填字段集合与迁移前一致