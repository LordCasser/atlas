...
"#,
    ...
    for (i, arg) in call_cs.args.iter().enumerate() {
        if arg.data_node_id.is_some() {
            ...
            assert_eq!(
                dn.kind,
                atlas::types::enums::DataNodeKind::CallArg,
...
"#,
    ...
    let call_targets: Vec<_> = nodes
        .iter()
        .filter(|n| n.kind == atlas::types::enums::DataNodeKind::CallTarget)
        .collect();
    assert!(
        call_targets.len() >= 2,
    ...
    let call_args: Vec<_> = nodes
        .iter()
        .filter(|n| n.kind == atlas::types::enums::DataNodeKind::CallArg)
        .collect();
    ...
    for ca in &call_args {
        if let Ok(edges) = store.find_dataflow_edges_by_source(&ca.id) {
            for e in &edges {
                if e.kind == atlas::types::enums::DataFlowKind::ArgToParam {
...
"#,
    ...
    let mut cross_fn_edges: Vec<(atlas::types::ids::DataNodeId, atlas::types::ids::DataNodeId)> =
        vec![];
    for node in &nodes {
        if let Ok(edges) = store.find_dataflow_edges_by_source(&node.id) {
            for edge in &edges {
