...
use atlas::cli::commands::{index, init};
use atlas::context::ContextBuilder;
use atlas::db::Store;
use atlas::graph::GraphEngine;
use atlas::mcp::tools::ToolRouter;
use atlas::search::SearchEngine;
use atlas::types::ids::{FileId, SymbolId};
...
fn call_tool(router: &ToolRouter, name: &str, args: Value) -> (Value, bool) {
    ...
    let text = match result.content.first() {
        Some(atlas::mcp::protocol::ContentBlock::Text { text }) => text.clone(),
        None => String::new(),
    };
...
"#,
    ...
    let (_tmp, router) = build_router(files);
    let store = Store::open(_tmp.path()).expect("open store");
