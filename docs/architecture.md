...
| Cangjie | DataflowInterproc | ✓ (branch/loop/`match`) | 0.65 | ✓ (ArgToParam + ReturnToCall, verified) | subject/conditionless match 的 direct unguarded wildcard 抑制 synthetic no-match；selector 保守流向 arm-scoped simple/tuple/enum-payload/type binding，guard/body 复用 identity；结构投影、guard control dependency 与复杂 exhaustiveness 保守；try/catch/finally continuation CFG |
