...
| Kotlin | DataflowInterproc | ✓ (branch/loop/`when`) | 0.67 | ✓ (ArgToParam + ReturnToCall) | 参数/local/catch binding 经 scope chain 解析，nested control scope 的同名 local 保留独立 identity；`when (val V = E)` 建模 initializer→subject binding，condition/guard/body 复用 scoped identity；try/catch/finally 与 `.use` normal/abrupt continuation CFG，cleanup 为确定性 LIFO；pinned grammar 不提取 extension receiver，smart-cast、definite-assignment、type/range projection 与 guard control dependency 保守 |
