...
> 当前基线：Atlas `1.7.0`、SQLite Schema V4、16 个 Cargo package、14 种默认语言、15 个 MCP 工具。版本号以 workspace manifests 为准，schema 以 `db::CURRENT_SCHEMA_VERSION` 为准，语言能力以 `LanguageCapabilityProfile` / `atlas doctor` 为准，MCP 工具面以 `make_all_tools()` 为准。
...
name-based 分组。写入在显式 RHS 完成求值后才激活，后续读取连接源码顺序上所有
已激活定义，因此 `x = x + 1` 的 RHS 不会读取当前写入，branch join 仍保留多个
may-reach 来源；线性 Trace 从候选中选择最近写入。Trace 在写节点优先选择显式值源，
在多操作数表达式中优先选择可解析的
ReturnToCall 桥，再选择变量/实参/字段等状态源而非字面量。该规则是跨语言共享的 source-order approximation；branch join 不等同于
CFG-aware SSA。
...
|----------|-------|:---:|:---:|:---:|------|
| TypeScript | DataflowInterproc | ✓ | 0.60 | ✓ (ArgToParam + ReturnToCall) | 普通/nested block 的同名 local 经 scope chain 保留独立 identity；direct identifier arithmetic/bitwise mutation 为 aggregate read-modify-write，logical `&&=`/`||=`/`??=` 保留 Read 0.75→Assign 0.90 的路径不敏感 may-provenance；`let/const` declaration destructuring 的 simple/renamed/nested/default/rest target 为 block-scoped binding，whole initializer 以 Assign 0.85 流向 target，computed key/default RHS 保持读取；object/array assignment destructuring 的 identifier leaf 复用 existing binding、不新建 BindingDef，whole RHS 以 Assign 0.85 流向 target；function/method/arrow parameter destructuring 的同类 leaf 为 function-scoped Parameter，共享顶层调用位置，whole argument 经 Full summary 或 Focus runtime ArgToParam 保守流向每个 leaf；`let/const` `for-of`/`for-in` simple/nested pattern 为 loop-scoped binding，existing-local assignment form 复用原 binding，whole iterable/object 以 Assign 0.65 流向 target，`for-await` 仅保留相同 aggregate provenance；logical RHS execution、exact property/index/element/key projection、destructuring/parameter default activation、`var` declaration/loop binding、member/subscript mutation/iteration/destructuring target、parallel assignment evaluation order、async scheduling 与 prefix/postfix result timing 保守；try/catch/finally path-isolated continuation CFG |
| JavaScript | DataflowInterproc | ✓ | 0.60 | ✓ (ArgToParam + ReturnToCall) | 共享 TS adapter 且保留 `javascript` identity；普通/nested block identity、direct-variable mutation/logical may-provenance、`let/const` declaration destructuring block binding/Assign 0.85 aggregate provenance、existing-local assignment destructuring binding reuse/Assign 0.85 aggregate provenance、function/method/arrow parameter destructuring function binding/shared argument position/ArgToParam 与 `let/const` `for-of`/`for-in` loop-scoped pattern/Assign 0.65 aggregate provenance 均与 TypeScript 对齐；logical RHS execution、exact property/index/element/key projection、destructuring/parameter default activation、`var` declaration/loop binding、member/subscript mutation/iteration/destructuring target、parallel assignment evaluation order、async scheduling 与 prefix/postfix result timing 保守；try/catch/finally continuation CFG |
| Python | DataflowInterproc | ✓ (branch/loop/`match`) | 0.72 | ✓ (ArgToParam + ReturnToCall) | module/function/class 同 namespace 重写共享 binding identity，普通 statement block 不造 scope，comprehension 隔离；direct identifier augmented assignment 建模为 aggregate read-modify-write Expr，previous value/explicit RHS→Expr 为 0.75、Expr→Local 为 0.90；attribute/subscript mutation target、in-place special-method dispatch 与 alias effect 保守；match subject 保守流向 capture/`as`/star binding 并贯通 guard/body；结构投影、post-match path-definedness、`global`/`nonlocal` 与动态 namespace mutation 保守；try/except/else/finally、`with` continuation CFG |
| Java | DataflowInterproc | ✓ | 0.75 | ✓ (ArgToParam + ReturnToCall) | parameter/local/foreach/catch/lambda 经 scope chain 解析，合法 sibling-block 同名 local 保留独立 identity；direct identifier compound/update expression 保留 aggregate read-modify-write provenance，previous value/explicit RHS→Expr 为 0.75、Expr→Local 为 0.90；member/array target、numeric promotion/boxing 与 prefix/postfix result timing 保守；`if` condition 的 direct/record `instanceof` 与 Java 21 arrow switch type/record capture 使用 scoped identity，sibling rule 同名 capture 不串线，tested value/selector 以 0.75 aggregate flow 流向 capture；Java 拒绝 overlapping local redeclaration，colon-style switch group、standalone/其他 flow-sensitive boolean context、exact record-component projection、compiler definite-assignment 与 guard dependency 保守；classic try/catch/finally 与 try-with-resources 组合的 normal/abrupt continuation CFG；direct `throw new T` 按 handler 顺序在首个无 guard 的语法精确匹配处截断；继承/alias/变量抛出仍保守；managed exit 先于 catch/finally，cleanup 为确定性 LIFO |
| C | DataflowInterproc | ✓ | 0.73 | ✓ (ArgToParam + ReturnToCall) | nested block 同名 local 经 scope chain 保留独立 identity；direct identifier compound/update expression 保留 aggregate read-modify-write provenance，previous value/explicit RHS→Expr 为 0.75、Expr→Local 为 0.90；field/subscript/pointer target 与 prefix/postfix result timing 保守；branch/loop/switch 与 direct same-function `Goto` CFG；函数指针 limited depth 3；computed/unresolved goto 保守终止 |
| C++ | DataflowInterproc | ✓ | 0.70 | ✓ (ArgToParam + ReturnToCall) | nested block 同名 local 经 scope chain 保留独立 identity；direct identifier compound/update expression 保留 aggregate read-modify-write provenance，previous value/explicit RHS→Expr 为 0.75、Expr→Local 为 0.90；field/subscript/pointer target、overloaded-operator dispatch/conversion 与 prefix/postfix result timing 保守；branch/loop/switch、direct same-function `Goto` 与 try/catch Exception CFG；lifecycle goto 清除不可证明的 branch context；cross-scope destruction、模板/重载/ADL、catch-type selection、implicit exception 不建模 |
| ArkTS | DataflowInterproc | ✓ (0.55) | 0.60 | ✓ (ArgToParam + ReturnToCall + AppStorage StateFlow) | TS grammar 下普通/nested block identity、direct-variable mutation/logical may-provenance、`let/const` declaration destructuring block binding/Assign 0.85 aggregate provenance、existing-local assignment destructuring binding reuse/Assign 0.85 aggregate provenance、function/method/arrow parameter destructuring function binding/shared argument position/ArgToParam 与 `let/const` `for-of`/`for-in` loop-scoped simple/nested pattern/Assign 0.65 aggregate provenance 均持久化为 `arkts` identity；logical RHS execution、exact property/index/element/key projection、destructuring/parameter default activation、`var` declaration/loop binding、member/subscript mutation/iteration/destructuring target、parallel assignment evaluation order 与 async scheduling 保守；等长 struct 归一化；named function/method branch-loop-switch 与 try/catch/finally continuation CFG 已验证；ArkUI trailing-block、嵌套 arrow callback ownership 仍是显式边界 |
| Go | DataflowInterproc | ✓ | 0.78 | ✓ (ArgToParam + ReturnToCall) | 普通/nested block 同名 local 经 scope chain 保留独立 identity；direct identifier compound/update expression 建模为 aggregate read-modify-write Expr，previous value/explicit RHS→Expr 为 0.75、Expr→Local 为 0.90；selector/index/pointer mutation target 保守；same-block mixed `:=` 复用 source-earlier local/函数体参数，新名字在声明后激活，switch/select clause 保持 sibling namespace，blank identifier 不造 binding/dataflow；type-switch guard value 保守流向每个 case/default implicit block 的 clause-local alias；identifier-only select receive 的 `:=` target 为 clause-local declaration、`=` target 复用 existing binding，whole receive operation 以 0.78 aggregate flow 流向各 target；case-type projection、function-literal ownership、exact receive-result component、non-identifier receive target 与 parallel-assignment evaluation order 保守；branch/loop/switch、`select` sibling 与 direct same-function `Goto` CFG；bounded path-sensitive defer stack 在 normal exit 通过 `Defer`→owner-matched `BlockExit` 做 LIFO cleanup，nested call argument effect 保持注册时执行；cyclic/over-budget defer 原子回退，panic/recover/Goexit 与复杂 anonymous deferred body 未建模 |
| C# | DataflowInterproc | ✓ | 0.72 | ✓ (ArgToParam + ReturnToCall) | direct identifier compound/update expression 保留 aggregate read-modify-write provenance，previous value/explicit RHS→Expr 为 0.75、Expr→Local 为 0.90；member/element target、`??=` conditional execution、overloaded/dynamic operator dispatch 与 prefix/postfix result timing 保守；`is`/switch 的 declaration/recursive/var pattern capture 与 switch parenthesized nested designation 经 scope chain 解析，switch arm 保留独立 identity；direct capture 接收 0.80 whole-subject flow，nested designation/property/list capture 接收 0.72 aggregate subject flow；exact property/index/positional projection、definite-assignment 与 guard dependency 保守；try/catch/finally 与 `using_statement` normal/abrupt continuation CFG；direct same-function `Goto` 退出按内到外执行 using/finally cleanup，非法 region crossing 被拒绝；direct `throw new T` 支持有序语法精确匹配截断，filter/继承/alias/变量抛出保持保守；cleanup 为确定性 LIFO |
| Rust | DataflowInterproc | ✓ (branch/loop/`match`/guard/`let-else`/`?`) | 0.70 | ✓ (ArgToParam + ReturnToCall) | named function/method tuple/struct/ref/nested parameter leaf 为 Parameter 并共享顶层 runtime `arg_index`，whole caller argument 经 Full summary/Focus runtime ArgToParam 保守流入；`self` 与 closure parameter 不映射到 enclosing named call，closure pattern 使用独立 lexical function scope 与本地 use-def；exact component projection、receiver→`self` 与 closure invocation resolution 保守；普通/nested block 同名 local 经 scope chain 保留独立 identity；ordinary `let`/`let-else` simple/nested pattern capture 复用 enclosing block scope，并在完整 declaration 结束后激活，使 initializer 与 alternative 保持 source-earlier binding identity；direct identifier initializer→Local 为 whole-value Assign 0.90，ordinary let initializer、match scrutinee 以及 source-ordered match-guard/`if`/`while` let-condition RHS 对 fixed tuple/tuple-struct/struct/slice-prefix capture 建 anonymous Expr access-path projection，value→projection 为 FieldLoad 0.80、projection→capture 为 Assign 0.90；direct identifier compound assignment 建模为 aggregate read-modify-write Expr，previous value/explicit RHS→Expr 为 0.75、Expr→Local 为 0.90；field/index/dereference mutation target 与 operator-trait dispatch/coercion 保守；guarded match arm 持久化 dispatch `CaseBranch`→guard `Branch`、guard `TrueBranch`→body 与 guard `FalseBranch`→shared `Join`，后续 arm 保持独立 sibling dispatch path；`let-else` 分离 success 与 abrupt alternative；`?` 保留 success 与 residual return-to-Exit，且不穿透 closure/async boundary；direct unguarded `_` 抑制 synthetic no-match；`if let` capture 贯通 later chain/success block 且在 `else` 前截止，`while let` capture 归属 loop scope；whole-pattern bare/ref/`@` capture 与 `..` 后 target 保留 aggregate Assign 0.75；runtime-length suffix projection、borrow/move mode、ordered pattern re-dispatch/pattern predicate proof、guard-to-value dataflow control dependency、单段常量歧义与 macro resolution/unwind 保守；Drop 仅为 function-exit heuristic |
| PHP | DataflowInterproc | ✓ (0.60) | 0.62 | ✓ (ArgToParam + ReturnToCall) | parameter、assignment-created local、`[]`/`list()` nested/keyed/by-reference destructuring target、foreach/catch/static declaration 与 explicit anonymous-function capture 经 file/function/method scope chain 解析；destructuring key expression 保持读取，assignment whole RHS→target 为 0.75、foreach collection→direct/nested target 为 0.65；direct file/function/method variable 的 `op=`、prefix/postfix `++`/`--` 建模为 aggregate read-modify-write Expr，previous value/explicit RHS→Expr 为 0.75、Expr→coalesced Local 为 0.90；匿名函数阻断未显式捕获的外层 local，anonymous-function dataflow 仍归入 enclosing named-function Focus unit；exact key/index projection、missing-key/null、reference-alias semantics、dynamic/non-variable destructuring/mutation target、`??=` conditional execution、prefix/postfix result timing、global alias、variable variable 与 arrow-function ownership 保守；function/method branch/loop/switch/elseif、fall-through、numeric break/continue、direct same-function `Goto`（含 inner-to-outer finally continuation）、try/catch/finally isolated continuations 与 return/throw→Exit 已验证；loop/switch entry 与 finally-clause crossing 被拒绝；direct `throw new T` 支持有序语法精确匹配截断，继承/alias/变量抛出仍保守 |
| Ruby | DataflowInterproc | ✓ (classic `case`/`when` + `case ... in` + modifier/lexical loops + `rescue/else/ensure` + block resource + `retry/redo`) | 0.65 | ✓ (ArgToParam + ReturnToCall) | simple assignment、flat/nested/rest multiple-assignment local target、parameter、rescue/for variable 与 case/in capture 使用 source-ordered method/module/class/block scope chain；block write 复用已存在的祖先 binding，新名字保持 block-local，block parameter 保持 shadowing，later outer assignment 不追溯吞并 earlier block local；direct identifier non-conditional operator assignment 建模为 aggregate read-modify-write Expr，previous value/explicit RHS→Expr 为 0.75、Expr→Local 为 0.90；receiver/index mutation target、`||=`/`&&=` conditional execution、operator-method dispatch 与 alias effect 保守；显式 multiple-assignment RHS list 按顶层位置映射，single aggregate RHS 与 nested/rest target 保留 group/slice 级保守流，结构投影、`to_ary` coercion、implicit `nil` fill、parallel evaluation order 与 numbered parameter 不建模；case/in sibling CFG 保留 implicit no-match Throw，unguarded capture/wildcard 可证明 exhaustive，且不消费 enclosing-loop break；subject 保守流向 bare/key-only/`=>`/rest capture，guard/body 复用 enclosing local identity，pin 保持读取；modifier while/until 区分 plain pre-test 与 `begin...end` post-test；lexical loop 与 modeled resource-block `redo` 回到 body entry；rescue-owned `retry` 经 nested cleanup 回到 begin dispatch；ordinary iterator/callback block、pattern structural projection、post-match path-definedness 与 yield 仍为 best-effort |
| Kotlin | DataflowInterproc | ✓ (branch/loop/`when`) | 0.67 | ✓ (ArgToParam + ReturnToCall) | 参数/local/catch binding 经 scope chain 解析，nested control scope 的同名 local 保留独立 identity；direct identifier compound/update expression 建模为 aggregate read-modify-write Expr，previous value/explicit RHS→Expr 为 0.75、Expr→Local 为 0.90；navigation/index mutation target、overloaded assignment/inc dispatch 与 prefix/postfix result timing 保守；`when (val V = E)` 建模 initializer→subject binding；typed local 的 simple `=` 写保留跨 branch join 的全部具体来源，裸声明只进入 BindingGraph，不伪造 DataFlow source；try/catch/finally 与 `.use` normal/abrupt continuation CFG，cleanup 为确定性 LIFO；pinned grammar 不提取 extension receiver，smart-cast、编译器级 variable-initialization proof、type/range projection 与 guard control dependency 保守 |
| Cangjie | DataflowInterproc | ✓ (branch/loop/`match`) | 0.65 | ✓ (ArgToParam + ReturnToCall, verified) | parameter/simple-local 的 nested block 同名变量经 scope chain 保留独立 identity；direct simple reassignment 保留 RHS→Local 0.90 provenance，且 LHS 不伪造 VariableUse；direct identifier non-conditional compound/postfix update 建模为 aggregate read-modify-write Expr，previous value/explicit RHS→Expr 为 0.75、Expr→Local 为 0.90；field/index target、`&&=`/`||=` conditional execution、operator dispatch/coercion、prefix/update-result timing 保守；subject/conditionless match 的 direct unguarded wildcard 抑制 synthetic no-match；selector 保守流向 arm-scoped simple/tuple/enum-payload/type binding，guard/body 复用 identity；simple/nested-tuple/enum-payload `for-in` capture 为 loop-scoped binding，enum constructor syntax 不建 binding，iterable 向每个 capture 提供 0.65 aggregate provenance，循环后恢复外层同名 identity；其他 tuple/destructuring、resource binding、exact iterator element/结构投影、pattern irrefutability 的编译器验证、guard control dependency 与复杂 exhaustiveness 保守；try/catch/finally continuation CFG |
...
`scope_aware_binding` 是经过路径证据约束的精度标记，不代表编译器级 name resolution。当前完整产品路径矩阵覆盖 Python、TypeScript、JavaScript、ArkTS、Java、C、C++、C#、Go、Rust、Kotlin、Cangjie、PHP、Ruby：直接 extraction 验证 `BindingDef`/`BindingUse`/`DataNode.binding_id`，SQLite Trace 验证持久化身份，Focus 与 full Index 验证同 unit 的 bindings/dataflow/CFG 等价。语言特有的 destructuring、pattern、callback、mixed declaration、smart-cast、definite-assignment 等边界仍以各 profile limitation 为准。
...
语义直接连接下一个 executable body 或 Join；相同三元组表示同一控制流事实，只持久化一次。
...
Branch，success 进入 Join，alternative 中显式 return/break/continue、unconditional `loop` 或
unqualified builtin `panic!`/`unreachable!`/`todo!`/`unimplemented!` 保持 abrupt；value 中的 `?`
是独立第三条 residual path。标准 panic-like macro 复用 Throw terminal，但不声称 panic unwind、
`catch_unwind` 或 macro shadowing/re-export resolution。guarded Rust `match` arm 复用既有 CFG
节点与边类型：match dispatch 以 `CaseBranch` 进入 guard `Branch`，`TrueBranch` 进入 arm body，
`FalseBranch` 结束当前 guarded-arm path 并进入 shared `Join`；后续 arm 仍是 dispatch 的独立
sibling path。这一抽象不证明源码顺序上的 pattern re-dispatch 或 pattern predicate，也不让
variable Trace 沿 guard-to-value control dependency 回溯。注释类 AST extra 在 statement dispatch 前
过滤，不能成为可执行 CFG entity。

Rust control-flow let pattern 不增加专用 IR：语言 adapter 将 `if_expression` 的 conditional
`ScopeDef` 截止到 consequence block 末尾，使 capture 可参与后续 let-chain condition 与 success
block lookup，但不会泄漏到 `else`；`while_expression` 复用既有 loop scope。每个 capture 的
`visible_from_byte` 等于所属 `let_condition` 末尾，以保持 chain 内源码顺序。match、guard、
`if` 与 `while` 的 let-condition value 共用同一 pattern projection/dataflow normalization，因而
Full Index 与 Focus on-demand dataflow 不需要第二套语义。

Rust parameter pattern 复用同一 root-aware classifier，不增加新的持久化 IR。命名
function/method 的每个 binding leaf 都是 `Parameter`，并把包含它的顶层 `parameter`
位置写入 `arg_index`；同一 tuple/struct/ref pattern 的所有 leaf 因而共享一个位置，
Full summary 与 Focus runtime bridge 都只消费这一个位置上的 whole argument。`self`
不是 runtime argument，closure parameter 也不能映射到 enclosing named function 的
callsite：一旦一个 callable 存在显式 `arg_index`，缺失位置就表示“不参与该 named-call
bridge”，而不是退回源码顺序。`closure_expression` 只补 lexical function scope 和本地
use-def，不伪造匿名 callable symbol 或 closure call resolution；精确 component projection、
receiver→`self` 与 closure invocation bridge 继续保持保守。

Rust ordinary `let`/`let-else` 同样不增加专用 scope 或持久化 IR：pattern leaf 进入当前最内层
lexical scope，`visible_from_byte` 统一设为完整 `let_declaration` 末尾。这样同作用域 shadow 的
initializer 与 `let-else` alternative 都只能命中 source-earlier binding，声明后的 use 才切换到
新 identity。ordinary let、match 与 control-flow let 复用同一个 root-aware pattern collector；
fixed tuple/tuple-struct/struct/slice-prefix 继续复用 anonymous Expr + `FieldLoad`/`Assign` 投影，
无法仅凭语法确定 component 的 whole-pattern/ref/`@`/post-rest target 退回 aggregate Assign。
Index Full 与 Focus on-demand dataflow 因此共享同一 extraction 结果，不在消费层重建激活或投影规则。
...
- 全部 14 种语言已覆盖各自声明的函数/方法 CFG 边界；PHP 覆盖 branch/loop/switch/elseif、numeric break/continue、direct same-function goto（含 finally continuation）与 return/throw terminal，Cangjie 额外覆盖 `match` sibling paths。Java、JS/TS/ArkTS、Go、Rust 与 Kotlin 的 grammar-visible lexical label 可解析 `break`/`continue`，包括穿过 finally/managed cleanup 的路径；C/C++/Go/C#/PHP 的 direct same-function goto/label 解析为专用 `Goto` edge，C# 与 PHP 各自执行语言规定的 cleanup 并拒绝非法 region crossing，未知或非直接目标终止本地路径。Python 的 unguarded syntax-irrefutable wildcard/capture/`as`/group/OR pattern，以及 Rust/Cangjie 的 direct unguarded wildcard，会抑制不可能的 synthetic no-match；Python capture/`as`/star、Ruby bare/key-only/`=>`/rest 与 Cangjie simple/tuple/enum-payload/type binding 从 match subject 接收保守 Assign 边，guard/body use 复用各自 namespace/arm identity；Rust guarded match arm 以 dispatch `CaseBranch`→guard `Branch`、guard `TrueBranch`→body、guard `FalseBranch`→shared `Join` 明确持久化控制分支，后续 arm 保持独立 sibling dispatch path；Rust ordinary let initializer、match scrutinee 与 source-ordered match-guard/`if`/`while` let-condition RHS 对 fixed tuple/tuple-struct/struct/slice-prefix capture 建立 anonymous Expr access-path，依次通过 FieldLoad 0.80 与 Assign 0.90 到达 capture；ordinary `let`/`let-else` capture 在完整 declaration 后激活，initializer/alternative 保持 source-earlier identity；`if let` capture 只贯通 later chain/success block 并在 `else` 前截止，`while let` capture 归属 loop scope，whole-pattern bare/ref/`@` capture 与 `..` 后 target 则保留 aggregate Assign 0.75；Ruby pin 保持读取。Java `if` condition 的 direct/record `instanceof` 与 Java 21 arrow switch type/record capture 使用 scoped identity，tested value/selector 以 0.75 aggregate Assign 流向 capture，colon group、其他 flow-sensitive boolean context、exact record projection 与 definite-assignment 保守；Cangjie simple/nested-tuple/enum-payload `for-in` capture 使用 loop-scoped identity，enum constructor syntax 不建 binding，iterable 向每个 capture 提供 aggregate Assign，exact iterator element/结构投影与 pattern irrefutability 的编译器验证仍保守。C++ 以 `Exception` edge 表达 try/catch 与 explicit throw；JavaScript/TypeScript/ArkTS、Java、C#、PHP、Python、Kotlin、Cangjie 和 Ruby 进一步以 path-isolated clone 表达 try/catch/except/finally-style continuation。Ruby modifier while/until 区分 plain pre-test 与 `begin...end` post-test，lexical/modifier-loop/resource-block `redo` 与 rescue-owned `retry` 分别持久化为 `Redo`/`Retry` edge，并与 nested cleanup 组合。Java/C#/PHP 的 direct object-created explicit throw 会按源码顺序连接 handler，并在首个无 guard 的语法精确匹配处截断；更早 handler 因继承未知而保留。更深的 structural/type-driven pattern exhaustiveness、Rust runtime-length suffix projection/borrow-move、ordered pattern re-dispatch/pattern predicate proof、guard-to-value dataflow control dependency、继承/alias catch selection、implicit exception、cleanup exception identity、Ruby ordinary iterator/callback block、computed goto、C# `goto case/default`、C++ cross-scope destruction 与 grammar-hidden label 仍以 capability limitations 为准。
...
`unsupported_language` gap，非 error 终态）。每个 transition 保留 owner-bound
`BranchFrame`：True/False/Case 沿既有 control owner 传播；`Exception` edge 通过 handler
target 的 try-dispatch predecessor 反查 owner，丢弃该 try 内已放弃的条件、保留外层条件，
并在 matching Join 弹栈。空分支/空 handler 直接进入 Join，不创建会泄漏到 region 外的
frame。MCP 将稳定的 `true`/`false`/`case`/`exception` 标签输出到 transition
`branch_context`；branch diff 不把 handler 当作普通 sibling 比较。
