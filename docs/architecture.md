...
| Cangjie | DataflowInterproc | ✓ (branch/loop/`match`) | 0.65 | ✓ (ArgToParam + ReturnToCall, verified) | parameter/simple-local 的 nested block 同名变量经 scope chain 保留独立 identity；subject/conditionless match 的 direct unguarded wildcard 抑制 synthetic no-match；selector 保守流向 arm-scoped simple/tuple/enum-payload/type binding，guard/body 复用 identity；simple/nested-tuple/enum-payload `for-in` capture 为 loop-scoped binding，enum constructor syntax 不建 binding，iterable 向每个 capture 提供 0.65 aggregate provenance，循环后恢复外层同名 identity；direct-variable mutation 尚未声明；其他 tuple/destructuring、resource binding、exact iterator element/结构投影、pattern irrefutability 的编译器验证、guard control dependency 与复杂 exhaustiveness 保守；try/catch/finally continuation CFG |
