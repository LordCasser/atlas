...
| PHP | DataflowInterproc | ✓ (0.60) | 0.62 | ✓ (ArgToParam + ReturnToCall) | parameter、assignment-created local、foreach/catch/static declaration 与 explicit anonymous-function capture 经 file/function/method scope chain 解析；匿名函数阻断未显式捕获的外层 local，foreach key/value 在 loop 后保持同一 identity；anonymous-function dataflow 仍归入 enclosing named-function Focus unit，global alias、variable variable、nested destructuring、compound/update assignment edge 与 arrow-function ownership 保守；function/method branch/loop/switch/elseif、fall-through、numeric break/continue、direct same-function `Goto`（含 inner-to-outer finally continuation）、try/catch/finally isolated continuations 与 return/throw→Exit 已验证；loop/switch entry 与 finally-clause crossing 被拒绝；direct `throw new T` 支持有序语法精确匹配截断，继承/alias/变量抛出仍保守 |
