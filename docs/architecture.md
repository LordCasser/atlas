...
| Ruby | DataflowInterproc | ✓ (classic `case`/`when` + modifier/lexical loops + `rescue/else/ensure` + block resource + `retry/redo`) | 0.65 | ✓ (ArgToParam + ReturnToCall) | modifier while/until 区分 plain pre-test 与 `begin...end` post-test，`next`/`redo`/`break` 分别进入 condition/body/join；lexical loop 与 modeled resource-block `redo` 回到 body entry；rescue-owned `retry` 经 nested cleanup 回到 begin dispatch；ordinary iterator/callback block 与 `case ... in` 未建模，yield 仍为 best-effort |
