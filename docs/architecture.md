...
|----------|-------|:---:|:---:|:---:|------|
| TypeScript | DataflowInterproc | ✓ | 0.60 | ✓ (ArgToParam + ReturnToCall) | 普通/nested block 的同名 local 经 scope chain 保留独立 identity；direct identifier arithmetic/bitwise mutation 为 aggregate read-modify-write，logical `&&=`/`||=`/`??=` 保留 Read 0.75→Assign 0.90 的路径不敏感 may-provenance；`let/const` declaration destructuring 的 simple/renamed/nested/default/rest target 为 block-scoped binding，whole initializer 以 Assign 0.85 流向 target，computed key/default RHS 保持读取；`let/const` `for-of`/`for-in` simple/nested pattern 为 loop-scoped binding，existing-local assignment form 复用原 binding，whole iterable/object 以 Assign 0.65 流向 target，`for-await` 仅保留相同 aggregate provenance；logical RHS execution、exact property/index/element/key projection、`var` declaration/loop binding、assignment/parameter destructuring、member/subscript mutation/iteration target、async scheduling 与 prefix/postfix result timing 保守；try/catch/finally path-isolated continuation CFG |
| JavaScript | DataflowInterproc | ✓ | 0.60 | ✓ (ArgToParam + ReturnToCall) | 共享 TS adapter 且保留 `javascript` identity；普通/nested block identity、direct-variable mutation/logical may-provenance、`let/const` declaration destructuring block binding/Assign 0.85 aggregate provenance 与 `let/const` `for-of`/`for-in` loop-scoped pattern/Assign 0.65 aggregate provenance 均与 TypeScript 对齐；logical RHS execution、exact property/index/element/key projection、`var` declaration/loop binding、assignment/parameter destructuring、member/subscript mutation/iteration target、async scheduling 与 prefix/postfix result timing 保守；try/catch/finally continuation CFG |
