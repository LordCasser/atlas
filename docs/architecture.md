...
| Rust | DataflowInterproc | ✓ (branch/loop/`match`/`let-else`/`?`) | 0.70 | ✓ (ArgToParam + ReturnToCall) | 普通/nested block 同名 local 经 scope chain 保留独立 identity；direct identifier compound assignment 建模为 aggregate read-modify-write Expr，previous value/explicit RHS→Expr 为 0.75、Expr→Local 为 0.90；field/index/dereference mutation target 与 operator-trait dispatch/coercion 保守；`let-else` 分离 success 与 abrupt alternative；`?` 保留 success 与 residual return-to-Exit，且不穿透 closure/async boundary；direct unguarded `_` 抑制 synthetic no-match；match scrutinee 以及 source-ordered match-guard/`if`/`while` let-condition RHS 对 fixed tuple/tuple-struct/struct/slice-prefix capture 建 anonymous Expr access-path projection，value→projection 为 FieldLoad 0.80、projection→capture 为 Assign 0.90；`if let` capture 贯通 later chain/success block 且在 `else` 前截止，`while let` capture 归属 loop scope；bare/`@` whole capture 与 `..` 后 target 保留 aggregate Assign 0.75；runtime-length suffix projection、borrow/move mode、condition/guard dependency、单段常量歧义与 macro resolution/unwind 保守；Drop 仅为 function-exit heuristic |
