...
|----------|-------|:---:|:---:|:---:|------|
| TypeScript | DataflowInterproc | ✓ | 0.60 | ✓ (ArgToParam + ReturnToCall) | 普通/nested block 的同名 local 经 scope chain 保留独立 identity；direct identifier 的 arithmetic/bitwise `op=`、prefix/postfix `++`/`--` 建模为 aggregate read-modify-write Expr；direct identifier `&&=`/`||=`/`??=` 以旧值和 RHS→Expr 的 Read 0.75、Expr→Local 的 Assign 0.90 建模路径不敏感 may-provenance，但不证明 RHS 执行；member/subscript mutation target、operator-specific truthiness/nullish control dependency、prefix/postfix result timing、nested destructuring 与 async path 保守；try/catch/finally path-isolated continuation CFG |
| JavaScript | DataflowInterproc | ✓ | 0.60 | ✓ (ArgToParam + ReturnToCall) | 共享 TS adapter 且保留 `javascript` identity；普通/nested block 的同名 local 经 scope chain 保留独立 identity；direct identifier arithmetic/bitwise mutation 与 logical `&&=`/`||=`/`??=` may-provenance 均与 TypeScript 对齐；logical assignment 不证明 RHS 执行；member/subscript target、operator-specific control dependency、prefix/postfix result timing、nested destructuring 与 async path 保守；try/catch/finally continuation CFG |
...
| ArkTS | DataflowInterproc | ✓ (0.55) | 0.60 | ✓ (ArgToParam + ReturnToCall + AppStorage StateFlow) | TS grammar 下普通/nested block 的同名 local 经 scope chain 保留独立 identity；direct identifier arithmetic/bitwise mutation 保留 aggregate read-modify-write provenance，logical `&&=`/`||=`/`??=` 保留路径不敏感 old-value/RHS may-provenance（Read 0.75、Assign 0.90），同时持久化为 `arkts` identity；logical assignment 不证明 RHS 执行；member/subscript target、operator-specific control dependency、prefix/postfix result timing、nested destructuring 与 async path 保守；等长 struct 归一化；named function/method branch-loop-switch 与 try/catch/finally continuation CFG 已验证；ArkUI trailing-block、嵌套 arrow callback ownership 仍是显式边界 |
