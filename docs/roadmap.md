...
- ✅ TypeScript、JavaScript、ArkTS、Java、C、C++、Go、Rust、Kotlin、Cangjie 的普通 block/sibling-block、PHP assignment/anonymous-function，以及 Ruby source-ordered block namespace
  ...
  arrow-function ownership 暂不伪造；Ruby block write 仅复用源码更早的祖先 binding；Go same-block mixed `:=` 已覆盖 local/函数体参数复用、声明后激活与 clause 隔离；其余语言特有的 pattern、mixed declaration、smart-cast 与
