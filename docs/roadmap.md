...
- Ensure `atlas doctor` exposes release-relevant state. ✅ Done: doctor
  prints Atlas version, Schema V3 state, canonical index mode from `Store`, compiled
  features, and per-language capability profiles; helper tests cover schema and
  index-mode reads.
- Compatibility: Schema V3 with no migration chain for older development schemas
...
> remaining language-specific guarded/pattern-binding dataflow (including Rust
> guard-let chains), and Ruby structural pattern
...
| **Schema V3 无迁移** | **已接受策略** | 架构 §6.1 明确"不保留旧 schema 运行时补丁路径"；`doctor` 存在；坏库 reject+重建指引已有。产品策略，非遗留。 |
...
**结论**：前 3 项已治理/修复/收敛，不应再列为债；Schema V3 是已接受策略；仅 Focus 布局为真债（低优先级）。
