...
- 所有已编译语言至少进入一条 Focus function-unit 与 full Index 的 bindings、dataflow（含 edge kind/confidence）、CFG
  ...
  destructuring block binding/aggregate provenance、`for-of`/`for-in` simple/nested
  pattern loop binding/aggregate provenance、Cangjie simple/nested-tuple/enum-payload
