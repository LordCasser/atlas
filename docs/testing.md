...
| Shared reaching definition / Trace value selection | `test_resolve_use_def_activates_assignment_after_rhs` + `fx_kotlin_branch_complete_late_assignment_persists_both_write_origins` + `fx2_multi_assignment_chain_complete` + `sem_c_multi_assignment_chain_complete` + `vfy_ts_canonical_provenance_path_field_to_return` | 写入只在显式 RHS 求值结束后激活；`x = x + 1` 的 RHS 不读取当前 write，后续 use 保留全部 source-ordered may-reach definitions，线性 Trace 从中选择最近写入；多操作数表达式优先选择可解析的 ReturnToCall 桥，再延续 state-bearing source，不因字面量或无跨函数语义的 CallTarget 提前终止；这是 source-order approximation，不冒充 CFG-aware SSA |
