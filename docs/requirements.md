...
|---|---|---|
| TypeScript | DataflowInterproc: 变量来源、call args、field access、return、CFG、跨函数 ArgToParam+ReturnToCall；nested block 同名 local 保留独立 binding identity；direct identifier 的 `op=`、prefix/postfix `++`/`--` 保留 aggregate read-modify-write provenance，mutation Expr→Local 为 0.90；direct identifier `&&=`/`||=`/`??=` 以旧值和 RHS→Expr 的 Read 0.75、Expr→Local 的 Assign 0.90 保留路径不敏感 may-provenance；try/catch/finally isolated continuations | 展示完整证据链；logical assignment 不得解释为 RHS 必然执行或精确 truthiness/nullish control dependency；member/subscript mutation target、prefix/postfix result timing、nested destructuring/async path 保持 limitation；跨函数结果标注 depth、summary/heuristic 和 confidence |
| JavaScript | 与 TypeScript 共用 grammar/dataflow 路径，DataflowInterproc；nested block 同名 local 保留独立 binding identity；direct identifier arithmetic/bitwise mutation 保留 aggregate read-modify-write provenance，`&&=`/`||=`/`??=` 保留与 TypeScript 相同的路径不敏感 old-value/RHS may-provenance；try/catch/finally isolated continuations | 展示同 TypeScript，但必须标注 `javascript`，不能混写成 `typescript`；不得据 logical assignment 推导 RHS 必然执行；member/subscript target、prefix/postfix result timing、nested destructuring/async path 保持 limitation |
...
| ArkTS | DataflowInterproc via TS grammar，confidence 0.60；普通 nested block 同名 local 保留独立 binding identity；direct identifier arithmetic/bitwise mutation 保留 aggregate read-modify-write provenance，`&&=`/`||=`/`??=` 保留路径不敏感 old-value/RHS may-provenance（Read 0.75、Assign 0.90）；CFG WithLimitations(0.55)；named function/method branch-loop-switch 与 try/catch/finally isolated continuations 已验证，ArkUI trailing-block/callback CFG 仍未建模 | 显示 `arkts via TypeScript grammar` provenance；不得据 logical assignment 推导 RHS 必然执行；member/subscript target、prefix/postfix result timing、nested destructuring/async path、ArkUI callback ownership 与 trailing-block internals 保持具体 limitation |
