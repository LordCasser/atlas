...
|---|---|---|
| TypeScript | DataflowInterproc: 变量来源、call args、field access、return、CFG、跨函数 ArgToParam+ReturnToCall；nested block 同名 local 保留独立 binding identity；try/catch/finally isolated continuations | 展示完整证据链；nested destructuring/async pattern 保持 limitation；跨函数结果标注 depth、summary/heuristic 和 confidence |
| JavaScript | 与 TypeScript 共用 JS grammar 路径，DataflowInterproc；nested block 同名 local 保留独立 binding identity；try/catch/finally isolated continuations | 展示同 TypeScript，但必须标注 `javascript`，不能混写成 `typescript`；nested destructuring/async pattern 保持 limitation |
...
| ArkTS | DataflowInterproc via TS grammar，confidence 0.60；普通 nested block 同名 local 保留独立 binding identity；CFG WithLimitations(0.55)；named function/method branch-loop-switch 与 try/catch/finally isolated continuations 已验证，ArkUI trailing-block/callback CFG 仍未建模 | 显示 `arkts via TypeScript grammar` provenance；ArkUI callback ownership 与 trailing-block internals 保持具体 limitation |
