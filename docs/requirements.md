...
|---|---|---|
| TypeScript | DataflowInterproc: 变量来源、call args、field access、return、CFG、跨函数 ArgToParam+ReturnToCall；nested block 同名 local 保留独立 binding identity；direct identifier 的 `op=`、prefix/postfix `++`/`--` 保留 aggregate read-modify-write provenance，mutation Expr→Local 为 0.90；try/catch/finally isolated continuations | 展示完整证据链；member/subscript mutation target、logical-assignment conditional execution、prefix/postfix result timing、nested destructuring/async path 保持 limitation；跨函数结果标注 depth、summary/heuristic 和 confidence |
| JavaScript | 与 TypeScript 共用 grammar/dataflow 路径，DataflowInterproc；nested block 同名 local 保留独立 binding identity；direct identifier mutation 保留 aggregate read-modify-write provenance；try/catch/finally isolated continuations | 展示同 TypeScript，但必须标注 `javascript`，不能混写成 `typescript`；member/subscript target、logical-assignment conditional execution、prefix/postfix result timing、nested destructuring/async path 保持 limitation |
...
| ArkTS | DataflowInterproc via TS grammar，confidence 0.60；普通 nested block 同名 local 保留独立 binding identity；direct identifier mutation 保留 aggregate read-modify-write provenance；CFG WithLimitations(0.55)；named function/method branch-loop-switch 与 try/catch/finally isolated continuations 已验证，ArkUI trailing-block/callback CFG 仍未建模 | 显示 `arkts via TypeScript grammar` provenance；member/subscript target、logical-assignment conditional execution、prefix/postfix result timing、nested destructuring/async path、ArkUI callback ownership 与 trailing-block internals 保持具体 limitation |
