...
Cangjie 已实现 **DataflowInterproc** 级别：基础定义/引用/导入、词法绑定、局部数据流、调用图、跨函数 summary，以及 branch/loop/`match` sibling CFG 均已实现。Parameter/simple-local 的 nested block 同名变量经 scope chain 保留独立 identity；direct unguarded wildcard 会抑制 synthetic no-match；match selector 保守流向 arm-scoped binding，guard/body 复用同一 identity；简单 `for-in` target 为 loop-scoped binding，iterable 向其提供 aggregate provenance，循环后恢复外层同名 identity。Tuple/destructuring（包括 `for-in` pattern）、resource binding、iterator element/结构投影、guard control dependency 与 guarded/复杂 pattern exhaustiveness 仍是显式边界。现为默认编译语言之一。
...
- Cangjie DataflowInterproc 抽取、调用图和 branch/loop/`match` CFG；parameter/simple-local nested block identity、direct unguarded wildcard exhaustiveness、selector→arm-scoped binding/guard/body identity，以及简单 `for-in` 的 loop-scoped target 与 iterable aggregate provenance 已建模；tuple/destructuring（包括 `for-in` pattern）、resource binding、iterator element/结构投影、guard control dependency 与复杂 exhaustiveness 暂未建模。
...
| Cangjie | DataflowInterproc: ArgToParam+ReturnToCall 已验证；parameter/simple-local nested block 同名变量保留独立 binding identity；branch/loop/`match` sibling CFG；direct unguarded wildcard exhaustiveness；selector 保守流向 simple/tuple/enum-payload/type capture，guard/body 复用 arm-scoped identity；简单 `for-in` target 为 loop-scoped binding，iterable 提供 0.65 aggregate provenance，循环后恢复外层同名 identity；try/catch/finally isolated continuations，confidence 0.65 | tuple/destructuring（包括 `for-in` pattern）、resource binding、iterator element/结构投影、guard control dependency 与 guarded/复杂 pattern exhaustiveness 保持 limitation |
