...
| Ruby | DataflowInterproc: ArgToParam+ReturnToCall；classic `case`/`when`、method-body/nested `rescue/else/ensure` 与 block-resource isolated continuations；lexical-loop/resource-block `redo` 和 rescue-owned `retry` 分别持久化为 `Redo`/`Retry` edge；modifier while/until 区分 plain pre-test 与 `begin...end` post-test，`next`/`redo`/`break` 分别进入 condition/body/join；confidence 0.65 | ordinary iterator/callback block、block/yield implicit calls 与 Ruby 2.7+ `case ... in` pattern matching 为 best-effort/未建模 |
