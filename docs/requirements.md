...
| Python | DataflowInterproc: module/function/class namespace identity、普通 block 不造 scope、comprehension 隔离；branch/loop/`match` sibling CFG；match subject→capture/`as`/star binding 并贯通 guard/body；unguarded syntax-irrefutable wildcard/capture/`as`/group/OR exhaustiveness；try/except/else/finally 与 `with` isolated continuations；ArgToParam+ReturnToCall，confidence 0.72 | guarded、structural/type-driven pattern exhaustiveness、结构投影、post-match path-definedness、`global`/`nonlocal`、exception-alias deletion、动态 namespace/call 保持 limitation |
...
| Go | DataflowInterproc: ArgToParam+ReturnToCall；普通/nested block 同名 local 保留独立 binding identity；same-block mixed `:=` 复用 source-earlier local/函数体参数，新名字声明后激活，switch/select clause 隔离且 blank identifier 不建事实；type-switch guard value→每个 case/default implicit block 的 clause-local alias；identifier-only select receive 的 `:=` target 为 clause-local declaration、`=` target 复用 existing binding，whole receive operation 以 0.78 aggregate flow 流向各 target；branch/loop/switch、`select` sibling、direct same-function `Goto` 与 bounded path-sensitive defer-stack CFG；normal exit 通过 `Defer`/owner-matched `BlockExit` 做 LIFO cleanup，nested call argument effect 在注册时执行，confidence 0.78 | type-switch case-type projection、function-literal ownership、exact receive-result component、non-identifier receive target、parallel-assignment evaluation order 与 computed/unresolved goto 保守；cyclic/over-budget defer 原子回退；panic/recover/Goexit、复杂 anonymous deferred body 与泛型 dataflow 显示 limitation |
...
| Ruby | DataflowInterproc: ArgToParam+ReturnToCall；simple assignment 与 flat/nested/rest multiple-assignment local target 共享 source-ordered method/module/class/block binding identity，显式 RHS list 按顶层位置流向 target；classic `case`/`when` 与 `case ... in` sibling CFG，refutable case/in 无 `else` 时保留 implicit Throw，unguarded capture/wildcard 抑制不可能的 no-match；case/in subject 保守流向 bare、key-only、`=>`、array/hash-rest capture，guard/body 复用 enclosing local identity，pin 保持读取；method-body/nested `rescue/else/ensure` 与 block-resource isolated continuations；lexical-loop/resource-block `redo` 和 rescue-owned `retry` 分别持久化为 `Redo`/`Retry` edge；modifier while/until 区分 plain pre-test 与 `begin...end` post-test，`next`/`redo`/`break` 分别进入 condition/body/join；confidence 0.65 | single aggregate RHS、nested/rest assignment 仅提供 group/slice 级保守流；assignment structural projection、`to_ary` coercion、implicit `nil` fill、parallel evaluation order、numbered parameter、ordinary iterator/callback block、block/yield implicit calls、pattern structural projection、post-match path-definedness 与更深 pattern exhaustiveness 保持保守 |
| Kotlin | DataflowInterproc: ArgToParam+ReturnToCall；参数/local/catch 与 nested control scope 同名 local 经 scope chain 保留独立 binding identity；branch/loop/`when` sibling CFG；`when (val V = E)` 建模 initializer→subject binding；typed local 的 simple `=` 写保留跨 branch join 的全部具体来源，裸声明不成为无来源 DataFlow definition；try/catch/finally 与 `.use` isolated continuations，confidence 0.67 | cleanup 为确定性 LIFO；pinned grammar 不提取 extension receiver binding；smart-cast、编译器级 variable-initialization proof、type/range projection、guard control dependency 与精确清理异常保持 limitation |
| Cangjie | DataflowInterproc: ArgToParam+ReturnToCall 已验证；parameter/simple-local nested block 同名变量保留独立 binding identity；branch/loop/`match` sibling CFG；direct unguarded wildcard exhaustiveness；selector 保守流向 simple/tuple/enum-payload/type capture，guard/body 复用 arm-scoped identity；simple/nested-tuple/enum-payload `for-in` capture 为 loop-scoped binding，enum constructor syntax 不建 binding，iterable 向每个 capture 提供 0.65 aggregate provenance，循环后恢复外层同名 identity；try/catch/finally isolated continuations，confidence 0.65 | 其他 tuple/destructuring、resource binding、exact iterator element/结构投影、pattern irrefutability 的编译器验证、guard control dependency 与 guarded/复杂 pattern exhaustiveness 保持 limitation |
