...
| ArkTS | DataflowInterproc via TS grammar，confidence 0.60；普通 nested block 同名 local 保留独立 binding identity；direct identifier arithmetic/bitwise mutation、`&&=`/`||=`/`??=` 路径不敏感 may-provenance，以及 `let/const` `for-of`/`for-in` simple/nested pattern loop binding/Assign 0.65 aggregate provenance 均沿 TS grammar 持久化为 `arkts`；CFG WithLimitations(0.55)；named function/method branch-loop-switch 与 try/catch/finally isolated continuations 已验证，ArkUI trailing-block/callback CFG 仍未建模 | 显示 `arkts via TypeScript grammar` provenance；不得据 logical assignment 推导 RHS 必然执行，也不得据 loop flow 推导精确 element/key 或 async scheduling；`var` loop binding、member/subscript mutation/iteration target、普通 declaration destructuring 剩余边角、ArkUI callback ownership 与 trailing-block internals 保持具体 limitation |
