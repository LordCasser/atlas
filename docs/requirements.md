...
- Cangjie DataflowInterproc 抽取、调用图和 branch/loop/`match` CFG；parameter/simple-local nested block identity、direct unguarded wildcard exhaustiveness 与 selector→arm-scoped binding、guard/body identity 已建模，tuple/destructuring、for-in、resource binding、结构投影、guard control dependency 与复杂 exhaustiveness 暂未建模。
...
| Cangjie | DataflowInterproc: ArgToParam+ReturnToCall 已验证；parameter/simple-local nested block 同名变量保留独立 binding identity；branch/loop/`match` sibling CFG；direct unguarded wildcard exhaustiveness；selector 保守流向 simple/tuple/enum-payload/type capture，guard/body 复用 arm-scoped identity；try/catch/finally isolated continuations，confidence 0.65 | tuple/destructuring、for-in、resource binding、结构投影、guard control dependency 与 guarded/复杂 pattern exhaustiveness 保持 limitation |
