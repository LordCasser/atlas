...
| Ruby | DataflowInterproc: ArgToParam+ReturnToCall；classic `case`/`when` 与 `case ... in` sibling CFG，refutable case/in 无 `else` 时保留 implicit Throw，unguarded capture/wildcard 抑制不可能的 no-match；method-body/nested `rescue/else/ensure` 与 block-resource isolated continuations；lexical-loop/resource-block `redo` 和 rescue-owned `retry` 分别持久化为 `Redo`/`Retry` edge；modifier while/until 区分 plain pre-test 与 `begin...end` post-test，`next`/`redo`/`break` 分别进入 condition/body/join；confidence 0.65 | ordinary iterator/callback block、block/yield implicit calls、case/in binding/deconstruction dataflow 与更深 pattern exhaustiveness 为 best-effort/未建模 |
