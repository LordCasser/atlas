...
use crate::types::ids::{CallsiteId, DataNodeId, SymbolId};
use crate::types::summary::{CallArgFlow, FunctionSummary, ParameterFlow, ReturnFlow};
...
"#;
        let file_id = crate::types::ids::FileId::generate("greet.ts");
...
"#;
        let file_id = crate::types::ids::FileId::generate("process.ts");
        let frontend = create_frontend(Language::TypeScript).unwrap();
...
"#;
        let file_id = crate::types::ids::FileId::generate("noop.ts");
        let frontend = create_frontend(Language::TypeScript).unwrap();
