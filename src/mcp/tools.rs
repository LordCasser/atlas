...
impl ToolRouter {
    ...
    fn handle_search(&self, args: &Value) -> String {
        ...
        let results = if let Some(k_str) = kind {
            match SymbolKind::from_str(k_str) {
                ...
                None => return format!("Unknown symbol kind: {}", k_str),
            }
        } else {
    ...
    fn handle_symbol(&self, args: &Value) -> String {
        ...
        let sym = match symbols.first() {
            Some(s) => s,
            None => return format!("Symbol not found: {}", qname),
        };

        let graph = (self.graph_fn)();
        let callers_count = graph.callers(&sid).callers.len();
        let callees_count = graph.callees(&sid).callees.len();
    ...
    fn handle_neighbors(&self, args: &Value) -> String {
    ...
    fn handle_callers(&self, args: &Value) -> String {
        let qname = get_str(args, "symbol");
        let limit = get_u64(args, "limit").unwrap_or(20) as usize;
    ...
    fn handle_callees(&self, args: &Value) -> String {
        let qname = get_str(args, "symbol");
        let limit = get_u64(args, "limit").unwrap_or(20) as usize;
    ...
    fn handle_impact(&self, args: &Value) -> String {
        let qname = get_str(args, "symbol");
    ...
    fn handle_context(&self, args: &Value) -> String {
        let qname = get_str(args, "symbol");
        let sid = match self.resolve_qname(qname) {
            Ok(id) => id,
            Err(e) => return e,
        ...
        match self.context.build_context_for_symbol(&sid) {
            Ok(view) => view.to_markdown(),
            Err(e) => format!("Context build error: {}", e),
        }
    }
}
