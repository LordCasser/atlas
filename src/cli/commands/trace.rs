...
pub fn run_point(
    ...
    let store = Arc::new(Store::open(root).context("Failed to open Atlas database")?);
    let engine = TraceEngine::new_with_root(store.clone(), root.to_path_buf());

    let file_id = match engine.resolve_file_id_with_root(root, file_path)? {
        Some(fid) => fid,
        None => {
            let resp: TraceQueryResponse<crate::types::trace::TracePoint> = TraceQueryResponse::err(
                "trace_point",
...
pub fn run_variable(
    ...
    let file_id = match engine.resolve_file_id_with_root(root, file_path)? {
        Some(fid) => fid,
        None => {
            let resp: TraceQueryResponse<crate::types::trace::TracePath> = TraceQueryResponse::err(
                "trace_variable",
                &format!("File not found in index: '{}'", file_path),
            );
            return json_or_err(
...
pub fn run_caller_path(
    ...
    let resp = if let Some(hex) = symbol_hex.filter(|h| !h.is_empty()) {
        let target_id: SymbolId = match hex.parse() {
            Ok(id) => id,
            Err(_) => {
                let resp: TraceQueryResponse<crate::types::caller_path::CallerChain> =
