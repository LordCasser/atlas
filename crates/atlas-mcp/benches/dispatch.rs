...
fn bench_call_tool_dispatch(c: &mut Criterion) {
    ...
    c.bench_function("dispatch_project_status", |b| {
        b.iter(|| {
            router.call_tool(black_box(&ctx), "project", black_box(&json!({"action": "status"})))
        })
    });

    c.bench_function("dispatch_domain_rules_list", |b| {
        b.iter(|| {
            router.call_tool(black_box(&ctx), "domain_rules", black_box(&json!({"action": "list"})))
        })
    });

    c.bench_function("dispatch_tasks", |b| {
        b.iter(|| {
            router.call_tool(black_box(&ctx), "tasks", black_box(&json!({})))
        })
    });
}
