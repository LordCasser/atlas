...
impl ToolRouter {
    ...
    fn handle_search_sync(&self, query: &str, limit: usize, kind: Option<&str>, scope: Option<&str>) -> (String, bool) {
        ...
        if entries.is_empty() && !self.has_indexed_files() {
            return ("No indexed files found — please run 'index' tool first.".into(), true);
        }
        ...
        let final_entries = if needs_lazy {
            ...
            match self.do_search(query, limit, kind) {
                Ok(e) => self.filter_by_scope(e, scope),
                Err(err) => { let mut s = format!("Search error after extraction: {}", err); s.push_str(self.index_not_run_guidance()); return (s, true); }
            }
        } else { entries };
        self.send_progress(1.0, &format!("Search complete ({} results)", final_entries.len()));
    ...
    fn handle_search_background(&self, query: &str, limit: usize, kind: Option<&str>, scope: Option<&str>) -> (String, bool) {
        let task_id = self.task_manager.create_task("search");
        ...
        std::thread::spawn(move || {
            ...
            let search_engine = atlas_engine::SearchEngine::new(store.clone(), std::sync::Arc::new(graph));
            let do_search = |q: &str, k: Option<&str>| -> anyhow::Result<Vec<atlas_engine::SearchResult>> {
                if let Some(k_str) = k { SymbolKind::from_str(k_str).map(|sk| search_engine.search_by_kind(q, sk, limit)).unwrap_or_else(|| search_engine.search_simple(q, limit)) }
                else { search_engine.search_simple(q, limit) }
            };
            let entries = match do_search(&q, k.as_deref()) {
                Ok(e) => e, Err(err) => { task_manager.fail_task(&tid, &format!("Search error: {}", err)); return; }
            };
            let needs_lazy = entries.is_empty() || entries.iter().any(|e| e.symbol.layer == "manifest");
            let final_entries = if needs_lazy {
                ...
                if let Ok(result) = lazy.ensure_structural_for_symbol(&q) {
                    if result.files_built > 0 { task_manager.update_progress(&tid, 80.0, &format!("Extracted {} files", result.files_built)); }
                }
                match do_search(&q, k.as_deref()) {
                    Ok(e) => e, Err(err) => { task_manager.fail_task(&tid, &format!("Re-search failed: {}", err)); return; }
                }
            } else { entries };
    ...
    fn do_search(&self, query: &str, limit: usize, kind: Option<&str>) -> anyhow::Result<Vec<atlas_engine::SearchResult>> {
        if let Some(k_str) = kind { SymbolKind::from_str(k_str).map(|k| self.search_engine().search_by_kind(query, k, limit)).unwrap_or_else(|| self.search_engine().search_simple(query, limit)) }
        else { self.search_engine().search_simple(query, limit) }
    }
    ...
    pub(crate) fn handle_symbol(&self, args: &serde_json::Value) -> (String, bool) {
        ...
        let symbols = match self.store.find_symbols_by_qname(qname) {
            Ok(s) => s, Err(e) => { let mut s = format!("Lookup error: {}", e); s.push_str(self.index_not_run_guidance()); return (s, true); }
        };
        let sym = match symbols.into_iter().next() {
            ...
            None => {
                ...
                match retry.into_iter().next() {
                    Some(s) => s,
                    None => { let mut s = format!("Symbol not found: {}", qname); s.push_str(self.index_not_run_guidance()); return (s, true); }
                }
            }
        };
