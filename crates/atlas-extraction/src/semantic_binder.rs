...
use atlas_types::ids::{FileId, ScopeId};
use atlas_types::{RawEdge, ReferenceUse, ScopeDef, SymbolDef};
...
impl SemanticBinder {
    ...
    fn innermost_scope(&self, range: atlas_types::TextRange) -> Option<ScopeId> {
        self.scopes
            .iter()
    ...
    pub fn contains_symbol(&self, id: &atlas_types::ids::SymbolId) -> bool {
    ...
    pub fn source_for_range(
        &self,
        range: atlas_types::TextRange,
    ) -> Option<atlas_types::ids::SymbolId> {
        self.registry.source_for_range(range)
    }
}

fn contains_range(outer: atlas_types::TextRange, inner: atlas_types::TextRange) -> bool {
    outer.start_byte <= inner.start_byte && outer.end_byte >= inner.end_byte
}

#[cfg(test)]
mod tests {
    use crate::SemanticBinder;
    use atlas_types::ids::{FileId, ScopeId, SymbolId};
    use atlas_types::{
    ...
    fn make_reference(text: &str, kind: ReferenceKind, range: TextRange) -> ReferenceUse {
        let file_id = make_file_id();
        ReferenceUse {
            id: atlas_types::ids::ReferenceId::generate(
                &file_id,
                None,
                range.start_byte,
                range.end_byte,
