        ...
        feature_overrides: &[
            (
                ...
                FeatureOverride::WithLimitations(
                    0.60,
                    &[
                        "scope-chain-aware binding via TS grammar; let/const declaration destructuring simple/renamed/nested/default/rest pattern captures are block-scoped; function/method/arrow parameter destructuring simple/renamed/nested/default/rest pattern captures are function-scoped and retain their top-level argument positions; let/const for-of/for-in simple and nested pattern captures are loop-scoped; assignment destructuring identifier leaves reuse existing bindings without creating declarations; computed keys and default RHS expressions remain reads; var declaration/loop binding semantics, member/subscript assignment targets, and ArkUI callback ownership remain conservative",
                    ],
                ),
            ),
            (
        ...
        feature_overrides: &[
            ...
            (
                ...
                FeatureOverride::WithLimitations(
                    0.70,
                    &[
                        "Control-flow graph with branch/loop body traversal and match sibling traversal implemented; guarded Rust match arms persist an explicit guard Branch: match dispatch enters it via CaseBranch, TrueBranch enters the arm body, and FalseBranch exits that guarded-arm path to the shared Join while later arms remain independent sibling dispatch paths; Rust let-else separates the successful match from explicit return/break/continue, unconditional-loop, and standalone unqualified builtin panic/unreachable/todo/unimplemented macro alternatives, and Rust ? preserves both the success continuation and residual return-to-Exit path outside nested closure/async boundaries; a direct unguarded wildcard arm suppresses the synthetic no-match path; ordinary let initializers, match scrutinees, and match-guard/if/while let-condition RHS values use fixed syntactic tuple/tuple-struct/struct/slice-prefix projections for captures used after activation; implicit Drop is a function-exit effect heuristic rather than path-sensitive lexical RAII; nested-expression macros, macro shadowing/re-exports, custom never-return macros, panic unwinding/catch_unwind, guarded/range exhaustiveness, ordered pattern re-dispatch/pattern predicate proof, runtime-length suffix projection, borrow/move modes, and guard-to-value dataflow control dependencies remain conservative",
                    ],
                ),
            ),
            (
                ...
                FeatureOverride::WithLimitations(
                    0.70,
                    &[
                        "resolved named calls use Full summaries or Focus runtime edges for ArgToParam (including shared positions for parameter-pattern leaves) and ReturnToCall; Rust self receiver bridging and closure invocation resolution remain conservative",
                    ],
                ),
            ),
        ],
