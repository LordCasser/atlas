...
mod tests {
    ...
    /// Regression (Item 10): `FileId::generate` hashes its input string verbatim.
    /// Absolute and relative paths produce *different* IDs. Callers must normalize
    /// to the project-relative form (strip the project root) before calling,
    /// typically via `std::path::Path::strip_prefix(workspace.root())`. The
    /// workspace layer guarantees this invariant.
    #[test]
    fn test_file_id_abs_vs_rel_not_equal() {
        let abs = FileId::generate("/home/user/project/src/main.ts");
        let rel = FileId::generate("src/main.ts");
        assert_ne!(
            abs, rel,
            "absolute vs relative paths must produce different IDs; caller must strip prefix"
        );
    }
