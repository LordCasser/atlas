...
impl LexicalBindingSpec for CAdapter {
    fn lexical_query(&self) -> &str {
        include_str!("../../queries/c/lexical.scm")
    }
    fn capability(&self) -> FeatureSupport {
        FeatureSupport::supported_with_limitations(0.65, vec!["name-based binding (no proper shadowing)"])
    }
    fn normalize(&self, ctx: NormalizeCtx<'_>, capture: Capture<'_>) -> Option<BindingDef> {
        normalize_c_lexical(&capture.name, capture.node, ctx.source, ctx.file_id)
    }
}

impl DataflowSpec for CAdapter {
    fn dataflow_builder_query(&self) -> &str {
        include_str!("../../queries/c/dataflow_builder.scm")
    }
    fn capability(&self) -> FeatureSupport {
...
fn c_binding_kind(capture_name: &str) -> Option<BindingKind> {
    match capture_name {
...
fn normalize_c_lexical(capture_name: &str, node: tree_sitter::Node, source: &str, file_id: FileId) -> Option<BindingDef> {
    let kind = c_binding_kind(capture_name)?;
    let name = node_text(node, source)?;
    let range = node_range(node);
    let scope_id = ScopeId::generate(&file_id, None::<&ScopeId>, kind.as_str(), range.start_byte);
...
fn find_call_expression_c(node: tree_sitter::Node) -> Option<tree_sitter::Node> {
...
fn normalize_c_dataflow_builder(capture_name: &str, node: tree_sitter::Node, source: &str, file_id: FileId) -> (Option<DataNode>, Option<DataFlowEdge>) {
    ...
    match capture_name {
        ...
        "df.assign_value" => {
            ...
            let callsite_id = find_call_expression_c(node).map(|ce| types::ids::CallsiteId::from_file_byte(&file_id, ce.start_byte() as u32));
