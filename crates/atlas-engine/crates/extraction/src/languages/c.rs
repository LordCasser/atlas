...
use crate::languages::shared::{make_binding_def, make_df_assign_field_target, make_df_assign_target, make_df_parameter, make_df_return_value, make_reference_use, make_scope_def, SymbolDefBuilder};
...
fn normalize_c_dataflow_builder(
    capture_name: &str,
    node: tree_sitter::Node,
    source: &str,
    file_id: FileId,
...
mod tests {
    ...
    fn test_dataflow_pointer_and_field_access() {
        ...
        assert!(
            node_hits > 0,
            "dataflow query should produce DataNodes for pointer/func C code"
        );
